...
const NameCard = ({ isDetail, dispatch, vm }) => <div className="m-nameCard">
  ...
  {isDetail ? <a onClick={() => dispatch({ type: 'about/toggleAll' })}>收起<i className="icon icon-dup"></i></a> : <a onClick={() => dispatch({ type: 'about/toggleAll' })}>查看详细资料<i className="icon icon-ddown"></i></a>}
</div>;
