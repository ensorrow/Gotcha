...
export default {
  getEvents: ({ page = 1, size = 5, tag_name }) => request(`/favorite_events?${tag_name ? `tag_name=${tag_name}&` : ''}page=${page}&size=${size}`),
  getDetail: event_id => request(`/events/${event_id}`, { ifAuth: true }),
  getWeekendEvents: ({ tag_name, page = 1, size = 5 }) => request(`/weekend_events?${tag_name ? `tag_name=${tag_name}&` : ''}page=${page}&size=${size}`),
